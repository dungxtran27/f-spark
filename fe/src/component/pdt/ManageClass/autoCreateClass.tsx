...
const AutoCreateClass: React.FC<AutoCreateClassProps> = ({ onSave }) => {
  ...
  const groups: Group[] = groupData?.data?.data?.group?.filter(
    (g: any) => !g?.class
  );
  ...
  const noGroupStudents: Student[] = studentsData?.data?.data?.students?.filter(
    (s: any) => !s?.group
  );
  ...
  const autoCreateClasses = () => {
    ...
    while (groupQueue.length > 0 || studentQueue.length > 0) {
      ...
      //gioi han cua mot lop la 36 goc sinh ~ 6 nhom
  ...
  const handleSave = async () => {
    try {
      ...
      if (previewClasses.some((cls) => cls.students.length < 10)) {
        message.error(
          "You must have at least 10 students in total to create a class."
        );
        return;
      }
  ...
  const handleDeleteClass = (className: string) => {
    ...
    message.success(
      `Class ${className} deleted successfully out of class preview!`
    );
  };
  ...
  const handleSaveClassCode = () => {
    ...
    setPreviewClasses((prevClasses) =>
      prevClasses.map((classes) =>
        classes.name === editingClassCode
          ? { ...classes, name: newClassCode }
          : classes
      )
    );
  ...
  const remainingGroups =
    groups.length -
    previewClasses.reduce((sum, cls) => sum + cls.groups.length, 0);

  const totalStudentsInitial =
    noGroupStudents.length +
    groups.reduce((sum, cls) => sum + cls.teamMembers.length, 0);
  const totalStudentsInPreview = previewClasses.reduce(
    ...
    <div className="">
      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2">
          ...
          >
            Remaining: {remainingGroups} Groups, {remainingStudents} Students no
            class yet
          </Tag>
        </div>
