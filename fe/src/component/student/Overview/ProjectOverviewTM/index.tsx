...
const ProjectOverviewWrapperTM: React.FC<{ groupId: string }> = ({ groupId }) => {
  ...
  const [customerRelationshipsText, setCustomerRelationshipsText] = useState("");
  ...
  const { data, isSuccess } = useQuery({
    ...
    queryFn: async () =>
      (await groupApi.getGroupById(groupId)).data.data,
  });
  useEffect(() => {
      if (isSuccess) {
        fetchSections()
  ...
  const getContentForCell = (rowId: string, colId: string) => {
    const cell =
      data?.customerJourneyMap?.cells.find(
  ...
  return (
    <div className="p-2 space-y-2 rounded">
      ...
      <div className="flex">
        ...
        {data?.customerJourneyMap?.cols.map(
          (col: any) => (
      ...
      {data?.customerJourneyMap?.rows.map(
