...
const getRequestDeadlineForDashBoard = async (req, res) => {
  try {
    const teacherId = req.decodedToken.role.id;
    const classId = req.params.classId
    if (!teacherId) {
    ...
    const requestDeadline = await RequestDeadlineRepository.getRequestDeadlineForDashBoard({ teacherId, classId })
    return res.status(200).json({ data: requestDeadline, totalItems: requestDeadline.totalItems });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
